...
export default function App() {
  ...
  useEffect(() => {
    if (mode === 'admin' || mode === 'admin-manage') loadAdmin().catch((error) => setMessage(error.message));
  }, [loadAdmin, mode]);

  const refreshAll = useCallback(async () => {
    await Promise.all([loadChildDay(), mode === 'admin' || mode === 'admin-manage' ? loadAdmin() : Promise.resolve()]);
